...
## 🔀 Multiple Server Nodes: Choose Your Timeline

The R3APΞX infrastructure features multiple distinct routing endpoints. You can configure your routing utility to point to the node that fits your playstyle:
...
### 🟢 Node 3: The Time Machine (Custom Override)
* **Endpoint URL:** `$project-r3apex-server$/override?date=YYYYMMDD` (e.g., `date=20180301`)
* **Mechanics:** Provides a completely customizable timestamp by injecting the specified date parameter into the server heartbeat.
* **Details:** Designed for players who want to pinpoint a specific historical era or accurately trigger past events. Time progresses at a normal 1:1 real-world scale starting exactly from your specified date.
---
